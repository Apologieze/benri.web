...
const Content: FC<Size> = ({ size }) => {
  return <p className={`flex items-center text-primary ${size === "sm" ? "gap-0.5 text-base sm:text-lg lg:text-xl" : "gap-1 text-4xl sm:text-5xl lg:text-6xl"}`}>
    Benri <Icon size={size} />
  </p>
}

const Icon: FC<Size> = ({ size }) => {
  return <svg viewBox="0 0 65 65" fill="none" xmlns="http://www.w3.org/2000/svg" className={`fill-primary ${size === "sm" ? "w-4 sm:w-5 lg:w-6" : "w-10 sm:w-12 lg:w-16"}`}>
